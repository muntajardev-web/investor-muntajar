...
function calculateUniversityRanking(university: UniversityWithProgram): number {
...
function calculateScholarshipAvailability(
  student: StudentProfileInput,
  university: UniversityWithProgram
): number {
...
function calculateEnglishTestMatch(
  student: StudentProfileInput,
  university: UniversityWithProgram
): number {
...
function getEligibilityReasons(
  ...
  if (student.ieltsOverall) {
    const minIelts = requirements
      .filter(r => r.type === "IELTS" && r.isMandatory)
      .reduce((min, r) => Math.min(min, parseFloat(r.minValue || "6.5")), 6.5);
...
function determineAdmissionChance(
  matchScore: number,
